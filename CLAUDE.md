...
- The menu-bar dot is drawn in Rust (`dot_icon` in `src-tauri/src/main.rs`) from the pose and count the web view reports via `set_tray_state`: four static looks (solid, hollow while items wait, muted, grey), never animated; the companion window starts hidden and follows the persisted `desktop` setting.
- The dot's dropdown is rebuilt in Rust from the summary the web view derives in `regula/src/tray.ts` and sends via `set_tray_info` (status, which is also the dot's tooltip; cockpit totals; last note with time; items waiting in the groups "Needs you" and "With the approver"; cockpit links; pause 1 h / until tomorrow or resume; then the settings: the desktop check item, click-through, links to the cockpit settings and help pages; every other setting lives in the cockpit). Menu ids `link:<n>` map to deep links and emit `regula:opened`; all wording, including the action labels, comes from `strings.ts`.
- `show_popup` is offered for every bubble except "working"; Rust shows the popup window (`popup.html`, `src/popup.ts`) under the tray icon only while the companion window is hidden. The popup shows and hides itself (6 s, 10 s with an Open button, held while hovered, or click).
